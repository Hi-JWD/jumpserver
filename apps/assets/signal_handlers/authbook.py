...
@receiver(post_init, sender=AuthBook)
def on_authbook_init_create(sender, instance, **kwargs):
    contrastive_attrs = ['allow_change_auth']
    for attr in contrastive_attrs:
        ori_attr = getattr(instance, attr)
        setattr(instance, '__original_%s' % attr, ori_attr)
...
def on_authbook_pre_create(sender, instance, **kwargs):
    ...
    change_version = True
    for attr in contrastive_attrs:
        current_attr = getattr(instance, attr)
        ori_attr = getattr(instance, '__original_%s' % attr)
        if current_attr != ori_attr:
