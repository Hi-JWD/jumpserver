...
class BecomeModule(BecomeBase):
    ...
    def check_password_prompt(self, b_output):
        ...
        prompts = self.GBK_SU_PROMPT_LOCALIZATIONS
        b_password_string_bgk = b"|".join((br'(\w+\'s )?' + to_bytes(p, 'gbk')) for p in prompts)
        b_password_string_bgk = b_password_string_bgk + to_bytes(u' ?(:|：) ?', 'gbk')
        b_su_prompt_gbk_localizations_re = re.compile(b_password_string_bgk, flags=re.IGNORECASE)

        return bool(b_su_prompt_localizations_re.match(b_output)) or bool(b_su_prompt_gbk_localizations_re.match(b_output))
